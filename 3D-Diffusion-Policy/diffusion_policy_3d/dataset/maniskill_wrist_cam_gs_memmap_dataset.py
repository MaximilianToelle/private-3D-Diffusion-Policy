...
class WristCamGSMemmapManiskillDataset(MultiMemmapDataset):
    def __init__(
        ...
        percentile_histogram_range=3.0,        # meters; bound on anchor-relative positions (relative_ee_pose stats)
        percentile_histogram_num_bins=2 ** 22,
        verbose=False,
    ):
        self.num_gaussians = num_gaussians
        self.percentile_histogram_range = percentile_histogram_range
        self.percentile_histogram_num_bins = percentile_histogram_num_bins
    ...
    def _get_normalizer_abs_joint_pos(self):
        ...
        if 'positions' in self.gs_params:
            normalizer['gs_positions'] = SingleFieldLinearNormalizer.create_center_radius(
                stats['gs_positions']['min'], stats['gs_positions']['max'])
        ...
        # --- Actions and Agent Proprioception: per-DOF min/max (individual physical
        # ranges), gripper dims already in [-1, 1] -> identity ---
        normalizer['action'] = SingleFieldLinearNormalizer.create_per_dof_min_max(
            stats['action']['min'], stats['action']['max'], num_identity_dims=1)
        normalizer['agent_proprio'] = SingleFieldLinearNormalizer.create_per_dof_min_max(
            stats['agent_proprio']['min'], stats['agent_proprio']['max'], num_identity_dims=2)
    ...
    def _get_normalizer_relative_ee_pose(self):
        ...
        Percentiles are computed per (timestep, feature_dim) independently, ALL through the
        shared StreamingPercentileHistogram (common/streaming_stats.py): one pass, constant
        memory. Retaining the raw values instead costs windows x T_obs x N x 3 floats
        (~90 GB at 500 trajectories -- OOM-killed the job), while the histogram's error is
        at most one bin width (1.4e-6 m at the configured range/bins), two orders of
        magnitude below the fp16 quantization the stored gaussian positions carry.
        """
        ...
        # =====================================================================
        # Phase 1: Stream every train window through the percentile histograms
        # =====================================================================
        # NOTE: we do not need to normalize rotations and gripper state but only positions,
        # so every histogram covers the 3 position dims of its timesteps (slot = t * 3 + d).
        def make_histogram(num_timesteps):
            return StreamingPercentileHistogram(
                num_slots=num_timesteps * 3,
                value_range=self.percentile_histogram_range,
                num_bins=self.percentile_histogram_num_bins)
        agent_proprio_histogram = make_histogram(T_obs)
        action_histogram = make_histogram(T_horizon)
        gs_positions_histogram = make_histogram(T_obs)
        ...
        for buf_i, buf in enumerate(self.replay_buffers):
            ...
            for idx in tqdm(range(len(normalization_sampler)), desc=desc):
                ...
                # reshape to (num_slots, num_values); row-major matches the slot order t*3+d
                agent_proprio_histogram.update(
                    torch_data['obs']['agent_proprio'][..., :3].reshape(-1, 1))    # (T_obs*3, 1)
                action_histogram.update(
                    torch_data['action'][..., :3].reshape(-1, 1))                  # (T_horizon*3, 1)
                gs_positions_histogram.update(
                    torch_data['obs']['gs_positions'].permute(0, 2, 1).reshape(T_obs * 3, -1))

        # =====================================================================
        # Phase 2: Read the 2%/98% percentiles off the histograms
        # =====================================================================
        normalizer = LinearNormalizer()
        lower_percentile = 2
        upper_percentile = 98

        def percentile_tensors(histogram, num_timesteps):
            """(num_timesteps, 3) lower/upper percentile tensors of one histogram."""
            return (histogram.percentile(lower_percentile).view(num_timesteps, 3),
                    histogram.percentile(upper_percentile).view(num_timesteps, 3))

        # --- agent_proprio (non-position dims stay at p02=-1/p98=+1: identity + clamp) ---
        p02_agent_proprio = torch.full((T_obs, D_agent_proprio), -1.0)
        p98_agent_proprio = torch.full((T_obs, D_agent_proprio), 1.0)
        p02_agent_proprio[:, :3], p98_agent_proprio[:, :3] = percentile_tensors(
            agent_proprio_histogram, T_obs)
        ...
        p02_action[:, :3], p98_action[:, :3] = percentile_tensors(action_histogram, T_horizon)
        ...
        # --- gs_positions ---
        p02_gs_positions, p98_gs_positions = percentile_tensors(gs_positions_histogram, T_obs)
    ...
    def _sample_to_data(self, sample, gaussian_selection):
        """

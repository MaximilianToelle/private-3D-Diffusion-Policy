...
class WristCamSpatialMemoryPCDMemmapManiskillDataset(MultiMemmapDataset):
    ...
    def get_normalizer(self, **kwargs):
        ...
        normalizer['point_cloud'] = SingleFieldLinearNormalizer.create_center_radius(
            pos_min, pos_max,
            input_stats_dict={'min': pos_min, 'max': pos_max, 'mean': pos_mean, 'std': pos_std})
        ...
        normalizer['action'] = SingleFieldLinearNormalizer.create_per_dof_min_max(
            action.min(dim=0)[0], action.max(dim=0)[0], num_identity_dims=1)
        normalizer['agent_proprio'] = SingleFieldLinearNormalizer.create_per_dof_min_max(
            agent_proprio.min(dim=0)[0], agent_proprio.max(dim=0)[0], num_identity_dims=2)

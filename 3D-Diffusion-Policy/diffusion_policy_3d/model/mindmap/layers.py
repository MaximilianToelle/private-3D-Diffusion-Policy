...
from diffusion_policy_3d.model.mindmap.multihead_custom_attention import MultiheadCustomAttention
...
class RelativeCrossAttentionLayer(nn.Module):
    ...
    def forward(
        self, query, value, diff_ts=None, query_pos=None, value_pos=None, key_padding_mask=None
    ):
        if diff_ts is not None:
...
class SelfAttentionLayer(nn.Module):
    ...
    def forward(self, query, diff_ts=None, query_pos=None, value_pos=None, key_padding_mask=None):
        if diff_ts is not None:
            adaln_query = self.adaln(query, diff_ts)
        else:
            adaln_query = query
...
class FFWRelativeCrossAttentionModule(nn.Module):
    def __init__(self, embedding_dim, num_attn_heads, num_layers, dropout=0.0, use_adaln=True):
        super().__init__()
        ...
        for _ in range(num_layers):
            self.attn_layers.append(
                RelativeCrossAttentionLayer(
                    embedding_dim, num_attn_heads, dropout=dropout, use_adaln=use_adaln
                )
            )
            self.ffw_layers.append(
                FeedforwardLayer(embedding_dim, embedding_dim, dropout=dropout, use_adaln=use_adaln)
            )
    ...
    def forward(
        ...
        for i in range(self.num_layers):
            query, weights = self.attn_layers[i](
                query, value, diff_ts, query_pos, value_pos, key_padding_mask
            )
...
class FFWRelativeSelfAttentionModule(nn.Module):
    ...
    def forward(
        ...
        for i in range(self.num_layers):
            query, weights = self.attn_layers[i](
                query, query, diff_ts, query_pos, query_pos, key_padding_mask
            )
            query = self.ffw_layers[i](query, diff_ts)
            output.append(query)
            output_weights.append(weights)
...
class FFWRelativeSelfCrossAttentionModule(nn.Module):
    ...
    def forward(
        self, query, context, diff_ts=None, query_pos=None, context_pos=None, key_padding_mask=None
    ):
        output = []
        output_weights_self_att = []
        output_weights_cross_att = []
        for i in range(self.num_layers):
            ...
            if self.cross_attn_layers[i] is not None:
                ...
                query, cross_weights = self.cross_attn_layers[i](
                    query, context, diff_ts, cur_query_pos, context_pos, key_padding_mask
                )
                output_weights_cross_att.append(cross_weights)
            # Self attend next
            query, self_weights = self.self_attn_layers[i](
                query, query, diff_ts, query_pos, query_pos
            )
            query = self.ffw_layers[i](query, diff_ts)
            output.append(query)
            output_weights_self_att.append(self_weights)

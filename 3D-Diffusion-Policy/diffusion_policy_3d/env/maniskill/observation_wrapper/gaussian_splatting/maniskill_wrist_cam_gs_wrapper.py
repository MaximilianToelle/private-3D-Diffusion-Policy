...
class WristCamGSManiskillDP3Wrapper(ManiSkillDP3BaseObsWrapper):
    ...
    keys, the per-step Gaussian sampling in `_get_perception_obs_dict`, and the per-episode
    buffers cleared in `_reset_perception_state`.
    """
    ...
    def __init__(
        ...
        super().__init__(env, representation_space, agent_proprio_dim, render_cam_name='wrist_cam')
